...
export const TypingAIMessage = ({ message, isActive, onComplete }: TypingAIMessageProps) => {
  ...
  return (
    <div className="flex flex-col items-start w-full max-w-xs sm:max-w-md lg:max-w-lg">
      ...
      <div className={`flex p-2.5 justify-center items-start gap-2.5 self-stretch rounded-lg border border-white backdrop-blur-sm min-h-[50px] transition-all duration-300 ${
        isTyping
        ...
        <div className="text-sm font-manrope text-chat-dark font-normal leading-relaxed whitespace-pre-line">
          {displayedText}
          {(isTyping || showCursor) && (
            <span className={`inline-block w-[2px] h-[1.2em] ml-[1px] transition-all duration-100 ${
              showCursor
